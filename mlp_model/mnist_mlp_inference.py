...
summit_inference.py
-------------------
Runs TFLite inference on the Ripple Summit (i686, Python 3.7).
...
Then run:
  python37 /var/rppl/storage/summit_inference.py
"""
...
MODEL_PATH   = os.path.join(BASE, "mnist_model.tflite")
SAMPLES_PATH = os.path.join(BASE, "test_samples.npy")
LABELS_PATH  = os.path.join(BASE, "test_labels.npy")

print("=" * 50)
print("Summit TFLite Inference Test")
print("=" * 50)
...
try:
    ...
    print(f"[OK] Loaded {len(samples)} test samples")
...
for i in range(len(samples)):
    x = samples[i : i + 1].astype(inp_details["dtype"])
...
if correct == n:
    print("\nAll samples correct. TFLite is working on the Summit.")
elif correct >= n * 0.8:
    print("\nMost samples correct. TFLite inference is functional.")
else:
